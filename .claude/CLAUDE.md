...
本アプリケーションは以下の非機能要件を満たすことを目標としています。詳細な診断レポートは `docs/non-functional-requirements-report.md` を参照してください。

### 1. 性能

| 項目 | 要件 | 現状 |
|------|------|------|
| レスポンスタイム | 一覧取得API：500ms以内 | ❌ 未設定 |
| クエリ効率 | N+1クエリを発生させない | ❌ **要改善（重大）** |
| アップロード制限 | ミドルウェアで制限（画像5MB、動画50MB） | 🟡 環境変数のみ |
| ページネーション | 20件単位 | ✅ 達成 |
| タイムアウト | APIリクエスト：30秒、DB接続：5秒 | ❌ 未設定 |

**最優先課題**: N+1クエリ問題
- タイムライン、投稿一覧で深刻なN+1クエリが発生（20件取得時に81クエリ）
- バッチクエリでの一括取得に変更が必要
...
| 項目 | 要件 | 現状 |
|------|------|------|
| CORS | フロントエンドのみ許可、credentials: true | 🟡 デフォルト設定 |
| 認証 | HttpOnly Cookie（SameSite=None; Secure） | ❌ Bearer Token |
| 入力検証 | 全入力にバリデーション | ✅ 達成 |
| レートリミット | 認証系API：5回/分、一般API：60回/分 | ❌ 未実装 |
| 脆弱性対策 | 定期的なセキュリティレビュー | ❌ 未実施 |
